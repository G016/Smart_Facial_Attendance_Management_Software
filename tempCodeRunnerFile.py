...
#     root.mainloop()
    ...
    def add_data(self):
        if self.var_Branch.get() == "Select Branch" or self.var_Session.get() == "Select Session" or self.var_Year.get() == "Select Year" or self.var_Semester.get() == "Select Semester":
            messagebox.showerror("Error", "All Fields Are Required")
        else:
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="Gaurav@716", database="face_recognition")
                my_cursor = conn.cursor()
                my_cursor.execute("INSERT INTO student (Branch, Session, Year, Semester, Student_Name, Roll_No, Gender, DOB, Email, Mobile_No) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (
                    self.var_Branch.get(),
                    self.var_Session.get(),
                    self.var_Year.get(),
                    self.var_Semester.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success", "Student details have been added")
            except Exception as es:
